...
module.exports = async (scryfallData, outDir, count) => {    
    ...
    if (card.sid === null || card.sid?.length === "") {
        throw new Error("WTF?", card);
    }
    ...
    let frontImageStatus = "new";
    if (fs.existsSync(path.join(outDir, card.oracleId, `${card.sid}-front.png`))){
        frontImageStatus = "old";
    }
    fs.appendFileSync(path.join(outDir, card.oracleId, "front-images"), `${frontImageStatus}|${card.date}|${card.sid}|${card.front}\n`);
    if (card.back) {
        let backImageStatus = "new";
        if (fs.existsSync(path.join(outDir, card.oracleId, `${card.sid}-back.png`))){
            backImageStatus = "old";
        }
        fs.appendFileSync(path.join(outDir, card.oracleId, "back-images"), `${backImageStatus}|${card.date}|${card.sid}|${card.back}\n`);
    }
