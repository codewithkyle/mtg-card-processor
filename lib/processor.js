...
module.exports = async (scryfallData, outDir, count) => {    
    ...
    if (fs.existsSync(path.join(outDir, card.oracleId, `${card.date}-front.png`))){
        frontImageStatus = "old";
    }
    fs.appendFileSync(path.join(outDir, card.oracleId, "front-images"), `${frontImageStatus}|${card.date}|${card.front}\n`);
    if (card.back) {
        let backImageStatus = "new";
        if (fs.existsSync(path.join(outDir, card.oracleId, `${card.date}-back.png`))){
            backImageStatus = "old";
        }
        fs.appendFileSync(path.join(outDir, card.oracleId, "back-images"), `${backImageStatus}|${card.date}|${card.back}\n`);
    }
