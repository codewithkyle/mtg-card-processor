...
function buildCardData(data){
    ...
    if (data?.["prices"]?.["usd"] !== null) {
        if (data["oracle_id"] == "5089ec1a-f881-4d55-af14-5d996171203b") console.log("usd", data?.["prices"]?.["usd"])
        card.price = parseFloat(data?.["prices"]?.["usd"]) * 100;
    }
    if (data?.["prices"]?.["tix"] !== null) {
        if (data["oracle_id"] == "5089ec1a-f881-4d55-af14-5d996171203b") console.log("tix", data?.["prices"]?.["tix"])
...
module.exports = async (scryfallData, outDir, count) => {    
    ...
    //if (card.oracleId == "5089ec1a-f881-4d55-af14-5d996171203b") console.log("new card", card)
